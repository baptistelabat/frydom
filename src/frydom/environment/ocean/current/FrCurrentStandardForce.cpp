...
namespace frydom {
    ...
    void FrCurrentStandardForce<OffshoreSystemType>::SetMaximumBreadth(double breadth) {
      assert(breadth > FLT_EPSILON);
      m_breadth = breadth;
    }

    template<typename OffshoreSystemType>
    void FrCurrentStandardForce<OffshoreSystemType>::SetDraft(double draft) {
      assert(draft > FLT_EPSILON);
      m_draft = draft;
    }

    template<typename OffshoreSystemType>
    void FrCurrentStandardForce<OffshoreSystemType>::SetLateralArea(double lateralArea) {
      assert(lateralArea > FLT_EPSILON);
      m_lateralArea = lateralArea;
    }

    template<typename OffshoreSystemType>
    void FrCurrentStandardForce<OffshoreSystemType>::SetTransverseArea(double transverseArea) {
      assert(transverseArea > FLT_EPSILON);
      m_transverseArea = transverseArea;
    }

    template<typename OffshoreSystemType>
    void FrCurrentStandardForce<OffshoreSystemType>::SetXCenter(double xCenter) {
      m_xCenter = xCenter;
    }

    template<typename OffshoreSystemType>
    void FrCurrentStandardForce<OffshoreSystemType>::SetLengthBetweenPerpendicular(double lpp) {
      assert(lpp > FLT_EPSILON);
      m_lpp = lpp;
    }

    template<typename OffshoreSystemType>
    void FrCurrentStandardForce<OffshoreSystemType>::Initialize() {
    ...
    template<typename OffshoreSystemType>
    void FrCurrentStandardForce<OffshoreSystemType>::Compute(double time) {

      Force force;
      Torque torque;

      auto rho = this->GetSystem()->GetEnvironment()->GetOcean()->GetDensity();

      FrFrame FrameAtCOG = this->m_body->GetFrameAtCOG(NWU);

      auto bodyVelocity = this->m_body->GetLinearVelocityInWorld(NWU);
      bodyVelocity.z() = 0.;

      Velocity fluxVelocityInBody =
          this->m_body->GetSystem()->GetEnvironment()->GetOcean()->GetCurrent()->GetRelativeVelocityInFrame(FrameAtCOG,
                                                                                                      bodyVelocity,
                                                                                                      NWU);
      ...
      this->m_body->GetRotation().GetCardanAngles_RADIANS(phi, theta, psi, NWU);
      auto bodyRotation = FrRotation(Direction(0., 0., 1.), psi, NWU);
      auto frame = FrFrame(this->m_body->GetCOGPositionInWorld(NWU), bodyRotation, NWU);

      auto worldForce = frame.ProjectVectorFrameInParent(force, NWU);
      auto worldTorque = frame.ProjectVectorFrameInParent(torque, NWU);

      this->SetForceTorqueInWorldAtPointInBody(worldForce, worldTorque, Position(m_xCenter, 0., 0.), NWU);
    }
