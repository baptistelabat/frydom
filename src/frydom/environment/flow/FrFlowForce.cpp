...
namespace frydom {
    ...
    FrFlowForce<OffshoreSystemType>::FrFlowForce(const std::string &jsonFile) {
      this->ReadTable(jsonFile);
    }

    template<typename OffshoreSystemType>
    void FrFlowForce<OffshoreSystemType>::ReadTable(const std::string &jsonFile) {
      ...
      // Delete double term
      if (std::abs(polar[0].first) < 10e-2 &&
          std::abs(polar.back().first - 2. * MU_PI) < 10e-2
          || std::abs(polar[0].first + MU_PI) < 10e-2 &&
    ...
    template<typename OffshoreSystemType>
    void FrFlowForce<OffshoreSystemType>::Compute(double time) {
...
//        auto frame = m_body->GetFrameAtCOG(NWU).ProjectToXYPlane(NWU);
    ...
    template<typename OffshoreSystemType>
    void FrCurrentForce<OffshoreSystemType>::Compute(double time) {

      FrFrame FrameAtCOG = this->m_body->GetFrameAtCOG(NWU);
      Velocity VelocityInWorldAtCOG = this->m_body->GetCOGLinearVelocityInWorld(NWU);

      this->m_fluxVelocityInBody =
          this->m_body->GetSystem()->GetEnvironment()->GetOcean()->GetCurrent()->GetRelativeVelocityInFrame(FrameAtCOG,
                                                                                                            VelocityInWorldAtCOG,
                                                                                                            NWU);

      FrFlowForce<OffshoreSystemType>::Compute(time);
    }

    template<typename OffshoreSystemType>
    void FrWindForce<OffshoreSystemType>::Compute(double time) {

      FrFrame FrameAtCOG = this->m_body->GetFrameAtCOG(NWU);
      Velocity VelocityInWorldAtCOG = this->m_body->GetCOGLinearVelocityInWorld(NWU);

      this->m_fluxVelocityInBody =
          this->m_body->GetSystem()->GetEnvironment()->GetAtmosphere()->GetWind()->GetRelativeVelocityInFrame(
              FrameAtCOG,
              VelocityInWorldAtCOG,
              NWU);
    ...
    make_current_force(const std::string &jsonFile, std::shared_ptr<FrBody<OffshoreSystemType>> body) {
      auto currentForce = std::make_shared<FrCurrentForce<OffshoreSystemType>>(jsonFile);
    ...
    make_wind_force(const std::string &jsonFile, std::shared_ptr<FrBody<OffshoreSystemType>> body) {
      auto windForce = std::make_shared<FrWindForce<OffshoreSystemType>>(jsonFile);
