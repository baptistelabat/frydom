...
namespace frydom {
  ...
  std::ostream &FrInertiaTensor::cout(std::ostream &os) const {  // OK
    ...
    os << "COG (m)   : " << m_cogPosition.GetX() << "\t" << m_cogPosition.GetY() << "\t" << m_cogPosition.GetZ()
       << std::endl;
...
//        return {newMass, Ixx, Iyy, Izz, Ixy, Ixz, Iyz, newCOG, NWU};
  ...
  void FrInertiaTensor::Add(const FrInertiaTensor &tensor, const FrFrame &frame2Toframe1) {
    ...
    Position newCOG = this->m_mass * this->m_cogPosition +
                      tensor.m_mass * frame2Toframe1.GetPointPositionInParent(tensor.m_cogPosition, NWU);
    newCOG /= newMass;
...
//        m_inertiaAtCOG += newMatrix;
  ...
  void
  FrInertiaTensor::SetInertiaTensorAtCOG(double mass, const FrInertiaTensor::InertiaMatrix &inertia,
                                         const Position &cogPos,
                                         FRAME_CONVENTION fc) {
    m_mass = mass;
    m_cogPosition = cogPos;
