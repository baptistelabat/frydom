...
namespace frydom {
    ...
    class FrCylindricalLink : public FrLink<OffshoreSystemType> {

     public:

      /// Constructor from two nodes and a pointer to the system.
      /// It automatically adds the link to the system
      FrCylindricalLink(const std::shared_ptr<FrNode<OffshoreSystemType>> &node1, const std::shared_ptr<FrNode<OffshoreSystemType>> &node2,
