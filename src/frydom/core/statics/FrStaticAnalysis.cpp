...
namespace frydom {
    ...
    FrStaticAnalysis<OffshoreSystemType>::FrStaticAnalysis(FrOffshoreSystem<OffshoreSystemType> *system) : m_system(
        system) {
      this->SetLogged(true);
    }

    template<typename OffshoreSystemType>
    void FrStaticAnalysis<OffshoreSystemType>::SetNbSteps(int nSteps) {
      m_nSteps = nSteps;
    }

    template<typename OffshoreSystemType>
    int FrStaticAnalysis<OffshoreSystemType>::GetNbSteps() const {
      return m_nSteps;
    }

    template<typename OffshoreSystemType>
    void FrStaticAnalysis<OffshoreSystemType>::SetNbIteration(int nIter) {
      m_nIterations = nIter;
    }

    template<typename OffshoreSystemType>
    int FrStaticAnalysis<OffshoreSystemType>::GetNbIteration() const {
      return m_nIterations;
    }

    template<typename OffshoreSystemType>
    void FrStaticAnalysis<OffshoreSystemType>::SetRelaxation(FrStaticAnalysis<OffshoreSystemType>::RELAXTYPE relax) {
      m_relax = relax;
    }

    template<typename OffshoreSystemType>
    typename FrStaticAnalysis<OffshoreSystemType>::RELAXTYPE
    FrStaticAnalysis<OffshoreSystemType>::GetRelaxation() const {
      return m_relax;
    }

    template<typename OffshoreSystemType>
    void FrStaticAnalysis<OffshoreSystemType>::SetTolerance(double tol) {
      m_tolerance = tol;
    }

    template<typename OffshoreSystemType>
    double FrStaticAnalysis<OffshoreSystemType>::GetTolerance() const {
      return m_tolerance;
    }

    template<typename OffshoreSystemType>
    void FrStaticAnalysis<OffshoreSystemType>::Initialize() {
      ...
      this->m_message->template AddField<double>("iteration", "-", "iteration of the static analysis",
                                                 [this]() { return c_iter; });

      this->m_message->template AddField<double>("residual", "-", "residual of the static analysis",
                                                 [this]() { return c_residual; });

      // Initialize the message
      FrObject<OffshoreSystemType>::InitializeLog(logPath);
    }

    template<typename OffshoreSystemType>
    void FrStaticAnalysis<OffshoreSystemType>::StepFinalize() {
    ...
    template<typename OffshoreSystemType>
    bool FrStaticAnalysis<OffshoreSystemType>::SolveStatic() {
...
//        for (int m_iter = 0; m_iter < nIter; m_iter++) {
    ...
    template<typename OffshoreSystemType>
    FrOffshoreSystem<OffshoreSystemType> *FrStaticAnalysis<OffshoreSystemType>::GetSystem() {
      return m_system;
    }
