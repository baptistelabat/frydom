...
namespace frydom {
    ...
    class FrHydroDB {
     ...
     private:
      ...
      std::vector<std::unique_ptr<FrBEMBody<OffshoreSystemType>>> m_bodies;   ///< List of BEM body database
      std::unique_ptr<FrHydroMapper<OffshoreSystemType>> m_mapper;            ///< Mapper between bodies and BEM body database
     ...
     public:
      ...
      void MaskReader(FrHDF5Reader &reader, std::string path, FrBEMBody<OffshoreSystemType> *BEMBody);

      void ModeReader(FrHDF5Reader &reader, std::string path, FrBEMBody<OffshoreSystemType> *BEMBody);

      void ExcitationReader(FrHDF5Reader &reader, std::string path, FrBEMBody<OffshoreSystemType> *BEMBody);

      void RadiationReader(FrHDF5Reader &reader, std::string path, FrBEMBody<OffshoreSystemType> *BEMBody);

      void WaveDriftReader(FrHDF5Reader &reader, std::string path, FrBEMBody<OffshoreSystemType> *BEMBody);

      void HydrostaticReader(FrHDF5Reader &reader, std::string path, FrBEMBody<OffshoreSystemType> *BEMBody);
      ...
      FrBEMBody<OffshoreSystemType> *NewBody(std::string bodyName);

      /// Return the BEM body database
      /// \param ibody Index of the BEM body
      /// \return BEM Body database
      FrBEMBody<OffshoreSystemType> *GetBody(int ibody);

      /// Return the BEM body database
      /// \param body frydom body object
      /// \return BEM body database
      FrBEMBody<OffshoreSystemType> *GetBody(std::shared_ptr<FrBody<OffshoreSystemType>> body);

      /// Return the BEM body database
      /// \param body frydom body object
      /// \return BEM body database
      FrBEMBody<OffshoreSystemType> *GetBody(FrBody<OffshoreSystemType> *body);

      /// Return the frydom body object
      /// \param body BEM body database
      /// \return frydom body
      FrBody<OffshoreSystemType> *GetBody(FrBEMBody<OffshoreSystemType> *body);

      /// Return the mapper between frydom body and BEM body database
      /// \return Mapper
      FrHydroMapper<OffshoreSystemType> *GetMapper();
      ...
      /// \param body body (frydom object)
      /// \param eqFrame Equilibrium frame of the corresponding body
      void Map(int iBEMBody, FrBody<OffshoreSystemType> *body,
               std::shared_ptr<FrEquilibriumFrame<OffshoreSystemType>> eqFrame);

      //std::vector<std::unique_ptr<FrBEMBody<OffshoreSystemType>>>::iterator begin() { return m_bodies.begin(); }

      //std::vector<std::unique_ptr<FrBEMBody<OffshoreSystemType>>>::iterator end() { return m_bodies.end(); }

      typename std::unordered_map<FrBEMBody<OffshoreSystemType> *, FrBody<OffshoreSystemType> *>::iterator begin();

      typename std::unordered_map<FrBEMBody<OffshoreSystemType> *, FrBody<OffshoreSystemType> *>::iterator end();
