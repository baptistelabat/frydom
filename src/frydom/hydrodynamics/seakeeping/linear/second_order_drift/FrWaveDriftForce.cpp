...
namespace frydom {
  ...
  FrWaveDriftForce<OffshoreSystemType>::FrWaveDriftForce(std::shared_ptr<FrHydroDB<OffshoreSystemType>> hdb)
      : m_hdb(hdb) {}

  template<typename OffshoreSystemType>
  void FrWaveDriftForce<OffshoreSystemType>::Compute(double time) {

    auto force = Force();
    auto torque = Torque();

    auto ocean = this->m_body->GetSystem()->GetEnvironment()->GetOcean();
    auto waveAmplitude = ocean->GetFreeSurface()->GetWaveField()->GetWaveAmplitudes();

    // Wave encounter frequencies
    auto eqFrame = m_hdb->GetMapper()->GetEquilibriumFrame(this->m_body);
  ...
  template<typename OffshoreSystemType>
  void FrWaveDriftForce<OffshoreSystemType>::Initialize() {
    FrForce<OffshoreSystemType>::Initialize();
    m_table = m_hdb->GetBody(this->m_body)->GetWaveDrift();
  }

  template<typename OffshoreSystemType>
  std::vector<double> FrWaveDriftForce<OffshoreSystemType>::GetRelativeWaveDir() const {
  ...
  template<typename OffshoreSystemType>
  std::vector<std::vector<double>> FrWaveDriftForce<OffshoreSystemType>::GetEncounterWaveFrequencies(Velocity speed) const {

    auto waveField = this->m_body->GetSystem()->GetEnvironment()->GetOcean()
