...
def plot_loads_all_wave_dir(data, w, DiffOrFKOrExc, ibody, iforce, beta, show = True, save = False, filename = "Loads.png"):
    ...
    if(DiffOrFKOrExc == 0): # Diffraction loads.
        ...
        # Amplitude.
        if(iforce <= 2):
            ylabel1 = r'$|F_{Diff}^{%s}(\omega, \beta)|$' % (Dof_notation[iforce] + "_" + str(ibody + 1))
        else:
            ylabel1 = r'$|M_{Diff}^{%s}(\omega, \beta)|$' % (Dof_notation[iforce] + "_" + str(ibody + 1))

        # Phase.:
        if(iforce <= 2):
            ylabel2 = r'$Arg\left[F_{Diff}^{%s}(\omega,\beta)\right] (deg)$' % (Dof_notation[iforce] + "_" + str(ibody + 1))
        else:
            ylabel2 = r'$Arg\left[F_{Diff}^{%s}(\omega,\beta)\right] (deg)$' % (Dof_notation[iforce] + "_" + str(ibody + 1))
        ...
        # Amplitude.
        if (iforce <= 2):
            ylabel1 = r'$|F_{FK}^{%s}(\omega, \beta)|$' % (Dof_notation[iforce] + "_" + str(ibody + 1))
        else:
            ylabel1 = r'$|M_{FK}^{%s}(\omega, \beta)|$' % (Dof_notation[iforce] + "_" + str(ibody + 1))

        # Phase.
        if (iforce <= 2):
            ylabel2 = r'$Arg\left[F_{FK}^{%s}(\omega,\beta)\right] (deg)$' % (Dof_notation[iforce] + "_" + str(ibody + 1))
        else:
            ylabel2 = r'$Arg\left[F_{FK}^{%s}(\omega,\beta)\right] (deg)$' % (Dof_notation[iforce] + "_" + str(ibody + 1))
        ...
        # Amplitude.
        if (iforce <= 2):
            ylabel1 = r'$|F_{Exc}^{%s}(\omega, \beta)|$' % (Dof_notation[iforce] + "_" + str(ibody + 1))
        else:
            ylabel1 = r'$|M_{Exc}^{%s}(\omega, \beta)|$' % (Dof_notation[iforce] + "_" + str(ibody + 1))

        # Phase.
        if (iforce <= 2):
            ylabel2 = r'$Arg\left[F_{Exc}^{%s}(\omega,\beta)\right] (deg)$' % (Dof_notation[iforce] + "_" + str(ibody + 1))
        else:
            ylabel2 = r'$Arg\left[F_{Exc}^{%s}(\omega,\beta)\right] (deg)$' % (Dof_notation[iforce] + "_" + str(ibody + 1))
