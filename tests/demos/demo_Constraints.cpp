...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  FrOffshoreSystem system("demo_Constraints");
  system.GetEnvironment()->ShowFreeSurface(false);
  system.GetEnvironment()->ShowSeabed(false);

  // Bodies definition
  auto fixedBody = system.NewBody("Fixed");
  fixedBody->SetFixedInWorld(true);
  fixedBody->SetColor(GoldenRod);

  auto movingBody = system.NewBody("Moving");
  movingBody->SetColor(CornflowerBlue);

  enum demo_cases {
    DistanceBetweenPoints, DistanceToAxis, PointOnPlane, PointOnLine, PlaneOnPlane, Perpendicular, Parallel
  };
...
//            fixedBody->AllowCollision(false);
      ...
      auto fixedNode = fixedBody->NewNode("fixedNode");
...
//            movingBody->AllowCollision(false);
      ...
      auto movingNode = movingBody->NewNode("movingNode");
      ...
      auto constraint = make_constraint_distance_between_points("distance_between_points", &system, fixedPoint,
                                                                movingPoint, autoDistance,
                                                                distance);
...
//            movingBody->AllowCollision(false);
      ...
      auto constraint = make_constraint_distance_to_axis("distance_to_axis", &system, fixedAxis, movingPoint, false,
                                                         distance);
...
//            fixedBody->AllowCollision(false);
      FrRotation fixedRotation;
      fixedRotation.RotX_DEGREES(25, fc);
...
//            movingBody->AllowCollision(false);
      ...
      double distance = 0.5;
      auto constraint = make_constraint_point_on_plane("point_on_plane", &system, fixedPlane, movingPoint, distance);

      auto freeBody = system.NewBody("freeBody");
    ...
    case PointOnLine: {
      ...
      fixedBody->AllowCollision(false);
      FrRotation fixedRotation;
      fixedRotation.RotX_DEGREES(45, fc);
      fixedBody->RotateAroundCOG(fixedRotation, fc);
      ...
      // Definition of the point on plane constraint
      auto constraint = make_constraint_point_on_line("point_on_line", &system, fixedLine, movingPoint);
    ...
    case PlaneOnPlane: {
      ...
      fixedBody->AllowCollision(false);
      FrRotation fixedRotation;
      fixedRotation.RotX_DEGREES(15, fc);
      fixedBody->RotateAroundCOG(fixedRotation, fc);
      ...
      bool flipped = false;
      double distance = 1;
      auto constraint = make_constraint_plane_on_plane("plane_on_plane", &system, fixedPlane, movingPlane, flipped,
                                                       distance);
    ...
    case Perpendicular: {
      ...
      auto constraint = make_constraint_perpendicular("perpendicular", &system, fixedAxis, movingAxis);
    ...
    case Parallel: {
      ...
      // Definition of the perpendicular constraint
      auto constraint = make_constraint_parallel("parallel", &system, fixedAxis, movingAxis);
