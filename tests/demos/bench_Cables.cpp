...
bool ValidationResults(const std::shared_ptr<FrCatenaryLine> CatenaryLine,
  ...
  //Analyse results
  double LengthRelError = 100. * abs(DynamicCable->GetStrainedLength() - CatenaryLine->GetStrainedLength()) /
                          CatenaryLine->GetStrainedLength();
  ...
  std::cout << " Relative error on the strained length = " << LengthRelError << "%, should be < "
            << TargetLenghtRelError << "%" << std::endl;
  std::cout << " Relative error on the tension = " << TensionRelError << "%, should be < " << TargetTensionRelError
            << "%" << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  FrOffshoreSystem system("bench_Cables");
  ...
  auto Node1 = system.GetWorldBody()->NewNode("Node1");
  Node1->SetPositionInBody(Position(-10., 0., -10.), NWU);
//    Node1->RotateAroundZInBody(MU_PI, NWU);
  auto Node2 = system.GetWorldBody()->NewNode("Node2");
