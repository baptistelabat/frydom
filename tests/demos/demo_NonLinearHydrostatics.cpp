...
int main(int argc, char *argv[]) {
  ...
  FrOffshoreSystem system("NonLinearHydrostatics");
...
//    makeItCylinder(cylinder, radius, h, mass);
  ...
  auto cylinderMesh = FrFileSystem::join(
      {system.config_file().GetDataFolder(), "ce/Cylinder/Free_cylinder_2900_panels.obj"});
  cylinder->AddMeshAsset(cylinderMesh);
...
//    meshOffset.SetRotation(cylRotation);
  ...
  // -- Hydrodynamic mesh
  auto CylinderMesh = make_hydro_mesh("CylinderMesh", cylinder, cylinderMesh,
...
//    auto linearHSForce = make_linear_hydrostatic_force(eqFrame, cylinder, "Free_cylinder_2900_panels.obj", FrFrame());
  ...
  // -- Hydrostatics NL
  auto forceHst = make_nonlinear_hydrostatic_force("nonlinear_hydrostatic", cylinder, CylinderMesh);
