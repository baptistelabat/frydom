...
int main(int argc, char *argv[]) {
  ...
  auto waveProbe = waveField->NewWaveProbe(0,
                                           0); // TODO: il faut importer la position depuis la HDB... (position de mesure de houle)
...
//    hydroDampingForce->SetSeakeepingDampings(1e5, 5e10, 5e10);
  ...
  FrHydroDB HDB = LoadHDB5(
      "frydom_hdb.h5");  // TODO: mettre la HDB dans system ??? (ca genere du coup le mapper en auto)
  ...
  auto radModel = std::make_shared<FrRadiationConvolutionModel>(&HDB,
                                                                &mySystem);  // TODO : Ajouter un addExcitationForceToHydroBody (et changer le nom de radiationModel...)
  ...
  excForce->SetWaveProbe(
      waveProbe);  // La wave probe devrait etre generee automatiquement par la classe FrLinearHydroModel car c'est une info de la HDB !!
