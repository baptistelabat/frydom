...
TEST(FrNode, Position) {
  ...
  FrOffshoreSystem system("test_FrNode");
  ...
  Acceleration testAcceleration =
      node->GetAccelerationInWorld(fc) - body->GetAccelerationInWorldAtPointInBody(NodePositionInBody, fc);
  ...
  testVelocity = node->GetFrameInWorld().GetRotation().Rotate(node->GetVelocityInNode(fc), fc) -
                 body->GetVelocityInWorldAtPointInBody(NodePositionInBody, fc);
  EXPECT_TRUE(testVelocity.isZero());
  if (not(testVelocity.isZero())) {
    std::cout << body->GetVelocityInWorldAtPointInBody(NodePositionInBody, fc) << std::endl;
  ...
  /// test for fun
  auto Node2BodyRotation = body->GetFrame().GetThisFrameRelativeTransform_WRT_OtherFrame(
      node->GetFrameInWorld()).GetRotation();
  testVelocity = Node2BodyRotation.Rotate(node->GetVelocityInNode(fc), fc) -
                 body->GetVelocityInBodyAtPointInBody(NodePositionInBody, fc);
  EXPECT_TRUE(testVelocity.isZero());
  if (not(testVelocity.isZero())) {
  ...
  testAcceleration = node->GetFrameInWorld().GetRotation().Rotate(node->GetAccelerationInNode(fc), fc) -
                     body->GetAccelerationInWorldAtPointInBody(NodePositionInBody, fc);
  EXPECT_TRUE(testAcceleration.isZero());
  if (not(testAcceleration.isZero())) {
    std::cout << body->GetAccelerationInWorldAtPointInBody(NodePositionInBody, fc) << std::endl;
  ...
  FrRotation NodeRotation;
  NodeRotation.SetCardanAngles_DEGREES(5., 9., 1., fc);
  FrFrame NodeFrame;
...
TEST(FrNode, FrNode_COGModification_Test) {
  ...
  // body COG position
  Position COGPosInBody;
  COGPosInBody.setRandom();
  ...
  Position nodePosInBody;
  nodePosInBody.setRandom();
  ...
  Position newCOGPosInBody;
  newCOGPosInBody.setRandom();
