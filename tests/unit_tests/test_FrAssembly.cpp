...
//    EXPECT_NEAR(testPos.norm(), 0., 1e-08);
  ...
  mathutils::Matrix33<double> testMat =
      assembly.GetInertiaTensor().GetInertiaMatrixAtCOG(fc) - bodyFull->GetInertiaTensor().GetInertiaMatrixAtCOG(fc);
...
//    std::cout<<body1->GetInertiaTensor()<<std::endl;
  ...
  if (d1 != 0 && d2 != 0) {
    ...
    FrFrame body2Frame(Position(-0.5 * L3 * (1. - d3), -0.5 * L2 * d2, 0.5 * L1 * d1),
                       FrRotation(Direction(0., 1., 0), 90 * DEG2RAD, fc), fc);
...
//        std::cout<<body2->GetInertiaTensor()<<std::endl;
    ...
    if (d1 < 1) {
      ...
      FrFrame body2Frame(Position(-0.5 * (1. - d3) * L3, -0.5 * d2 * L2, -0.5 * d1 * L1),
                         FrRotation(Direction(0., 1., 0), 90 * DEG2RAD, fc), fc);
...
//            std::cout<<body3->GetInertiaTensor()<<std::endl;
  ...
  FrFrame body1Frame(Position(0.5 * L1, 0.5 * L2, 0.5 * d3 * L3), FrRotation(), fc);
  FrFrame body4Frame(Position(0.5 * L1, -0.5 * (1. - d3) * L3, -0.5 * (1. - d2) * L2),
                     FrRotation(Direction(1., 0., 0), -90 * DEG2RAD, fc), fc);
